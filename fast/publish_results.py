...
def plan(tag):
    ...
    items += [(f"ov3/results_{tag}.json", "EVAL12: every metric, every arm, every condition"),
              ...
              (f"ov3/latency_{tag}.json", "inference latency per arm, fp32 eager on the eval laptop"),
              (f"ov3/gated_{tag}.json", "high-band deficit by frame loudness (loud / mid / quiet), audit/gated_arms.py"),
