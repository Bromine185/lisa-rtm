...
const DICT = {
  ...
  "tour.2.p": ["Concatenate eight Gaussian channels at the input and train the same network under the energy score, two draws per step. The score is strictly proper, so the network converges to p(y|x) rather than its mean. One forward pass is one sample; sixteen of them give the spread. With the noise at zero it is LISA exactly (+896 weights). The run's own evaluation says the spread is usable but not yet calibrated — the truth sits above all sixteen draws more often than it should.",
               "入力に 8 本のガウス雑音チャネルを連結し、同じネットワークをエネルギースコア（1ステップに2サンプル）で学習します。スコアは厳密に proper なので、ネットワークは平均ではなく p(y|x) に収束します。順伝播 1 回が 1 サンプルで、16 回で分布の広がりが得られます。雑音をゼロにすれば LISA そのもの（+896 重み）です。ただし本実行の評価では、この広がりは使えるものの、まだ較正されていません — 原音が 16 サンプルすべてを上回る頻度が理想より高いのです。"],
